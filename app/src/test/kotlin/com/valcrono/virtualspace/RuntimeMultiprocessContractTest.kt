...
class RuntimeServiceOwnsRunningVirtualAppTest { @Test fun serviceOwnsRuntime() { val s = source("app/src/main/java/com/valcrono/virtualspace/RuntimeSlots.kt"); assertTrue(s.contains("private var running: RunningVirtualApp?")); assertFalse(source("app/src/main/java/com/valcrono/virtualspace/ProxyActivity.kt").contains("RunningVirtualApp")) } }
...
class StopDestroysRuntimeInServiceTest { @Test fun orderedCallbacksExist() { val s = source("app/src/main/java/com/valcrono/virtualspace/RuntimeSlots.kt"); assertTrue(s.indexOf("onPause()") < s.indexOf("onStop()")); assertTrue(s.indexOf("onStop()") < s.indexOf("onDestroy()")); assertTrue(s.contains("runtimeSlots().release")) } }
