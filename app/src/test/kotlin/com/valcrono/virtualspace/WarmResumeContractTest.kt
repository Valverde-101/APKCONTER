...
private fun src(path: String) = java.io.File(path).readText()
...
class WarmResumeReturnsCachedContentTest { @Test fun cachedContentReturnedByAttach() { assertTrue(src("app/src/main/java/com/valcrono/virtualspace/RuntimeSlots.kt").contains("cachedContent ?: running?.entry?.createContent()")) } }
...
class RuntimeAckRequiresSessionAndSlotUpdateTest { @Test fun ackChecksBothRows() { assertTrue(src("app/src/main/java/com/valcrono/virtualspace/RuntimeSlots.kt").contains("ack.sessionChanged == 1 && ack.slotChanged == 1")) } }
...
class ActivityTaskIsReusedTest { @Test fun moveTaskToFrontIsUsed() { assertTrue(src("app/src/main/java/com/valcrono/virtualspace/MainActivity.kt").contains("moveTaskToFront")) } }
