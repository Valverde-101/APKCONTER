...
@Dao interface RuntimeSlotDao {
    ...
    @Query("SELECT * FROM runtime_slots WHERE state='FREE' AND sessionId IS NULL AND packageName IS NULL AND launchAttemptId IS NULL AND hostPid IS NULL AND reclaimInProgress=0 AND slotId IN (:enabledSlotIds) ORDER BY slotId LIMIT 1") suspend fun firstFree(enabledSlotIds: List<String>): RuntimeSlotEntity?
    ...
    @Query("UPDATE runtime_slots SET state='FREE', packageName=NULL, virtualUserId=NULL, sessionId=NULL, launchAttemptId=NULL, hostPid=NULL, assignedAt=NULL, startedAt=NULL, lastHeartbeatAt=NULL, lastHeartbeatElapsedRealtime=NULL, lastHeartbeatWallClock=NULL, stoppedAt=:now, pssBytes=NULL, errorCode=NULL, errorMessage=NULL, taskId=NULL, activityInstanceId=NULL, activityLastAttachedAt=NULL, activityAttached=0, binderAlive=0, reclaimInProgress=0, lastReclaimReason=:reason, lastReclaimAt=:now WHERE sessionId IS NULL AND packageName IS NULL AND launchAttemptId IS NULL AND hostPid IS NULL AND state != 'FREE'") suspend fun normalizeEmptySlots(reason:String, now:Long): Int
