...
@Dao interface VirtualRuntimeDao {
    ...
    @Query("SELECT * FROM virtual_runtime_sessions WHERE state='STARTING' AND createdAt < :deadline") suspend fun staleStarting(deadline:Long): List<VirtualRuntimeSessionEntity>
    ...
    @Query("UPDATE virtual_runtime_sessions SET state='ERROR',stoppedAt=:now,lastActivityAt=:now,errorCode='ERROR_START_TIMEOUT',sanitizedError='La aplicación tardó más de 15 segundos en iniciar. Puedes reintentar.',classLoaderState='FAILED' WHERE state='STARTING' AND createdAt < :deadline") suspend fun timeoutStarting(deadline:Long,now:Long)
    @Query("UPDATE virtual_runtime_sessions SET state='ERROR',stoppedAt=:now,lastActivityAt=:now,errorCode='ERROR_STALE_STARTING',sanitizedError='Inicio anterior limpiado al abrir VirtualSpace. Puedes reintentar.',classLoaderState='FAILED' WHERE state='STARTING'") suspend fun markStaleStarting(now:Long)
    @Query("UPDATE virtual_runtime_sessions SET state='STOPPED',stoppedAt=:now,lastActivityAt=:now,errorCode='STOPPED_PROCESS_LOST',sanitizedError='El proceso anterior ya no existe.' WHERE state IN ('ACTIVE','PAUSED')") suspend fun markProcessLost(now:Long)
    @Query("UPDATE virtual_runtime_sessions SET state='ERROR',stoppedAt=:now,lastActivityAt=:now,errorCode='ERROR_STALE_STARTING',sanitizedError='Inicio anterior limpiado al abrir VirtualSpace.',classLoaderState='FAILED' WHERE state='STARTING'") suspend fun cleanupStarting(now:Long)
    ...
    @Query("UPDATE virtual_runtime_sessions SET state='STOPPED',stoppedAt=:now,lastActivityAt=:now WHERE state != 'STOPPED'") suspend fun stopAll(now:Long)
}
