...
@Dao interface VirtualRuntimeDao {
    ...
    @Query("UPDATE virtual_runtime_sessions SET classLoaderState='LOADED', launchPhase='CLASSLOADER_READY', lastHeartbeatAt=:now, lastActivityAt=:now WHERE sessionId=:sessionId AND currentLaunchAttemptId=:attemptId AND state='STARTING'") suspend fun markClassLoaderLoaded(sessionId:String, attemptId:String, now:Long): Int
...
val MIGRATION_16_17 = object : Migration(16, 17) {
    override fun migrate(db: SupportSQLiteDatabase) {
        fun add(sql: String) = runCatching { db.execSQL(sql) }
        add("ALTER TABLE virtual_packages ADD COLUMN apkVirtualPath TEXT NOT NULL DEFAULT ''")
        add("ALTER TABLE virtual_packages ADD COLUMN apkArtifactVersion INTEGER NOT NULL DEFAULT 1")
        add("ALTER TABLE virtual_packages ADD COLUMN apkIntegrityState TEXT NOT NULL DEFAULT 'UNKNOWN'")
        add("ALTER TABLE virtual_packages ADD COLUMN apkLastVerifiedAt INTEGER")
