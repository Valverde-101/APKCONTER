...
@Dao interface VirtualRuntimeDao {
    ...
    @Query("UPDATE virtual_runtime_sessions SET state=CASE WHEN :slotState IN ('ACTIVE_FOREGROUND','ACTIVE_BACKGROUND','OCCUPIED') THEN 'ACTIVE' ELSE state END, launchPhase=:phase, lastHeartbeatAt=:now, lastActivityAt=CASE WHEN :slotState IN ('ACTIVE_FOREGROUND','ACTIVE_BACKGROUND','OCCUPIED') THEN :now ELSE lastActivityAt END, hostPid=:pid, classLoaderState='LOADED', hasReachedActiveAck=1, lastActiveAtElapsed=:elapsed, lastHeartbeatSequence=lastHeartbeatSequence+1, errorCode=NULL, sanitizedError=NULL WHERE sessionId=:sessionId AND currentLaunchAttemptId=:attemptId AND hasReachedActiveAck=1") suspend fun heartbeatRuntimeSession(sessionId:String, attemptId:String, slotState:String, phase:String, now:Long, elapsed:Long, pid:Int): Int
