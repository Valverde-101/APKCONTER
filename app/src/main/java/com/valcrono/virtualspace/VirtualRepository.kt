...
class VirtualRepository(private val context: Context) {
    ...
    suspend fun validateOnStartup() {
        db.packages().getAll().forEach { pkg -> runCatching { reprocessImportedPackage(pkg) }.onFailure { VLog.w("Repository", "reprocess failed package=${pkg.packageName}: ${it.message}") } }
    }
