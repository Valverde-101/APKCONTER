...
class SQLiteViewerRepository(private val context: Context, private val vfs: VirtualFileSystem) {
    ...
    private fun SQLiteDatabase.indexRows(): List<SQLiteIndexSummary> = rawQuery("SELECT name, tbl_name, [unique], sql FROM sqlite_master WHERE type='index' ORDER BY name", null).use { c -> buildList { while (c.moveToNext()) add(SQLiteIndexSummary(c.getString(0), c.getString(1), c.getInt(2) != 0, if (c.isNull(3)) null else c.getString(3))) } }
    private fun SQLiteDatabase.tableSummary(table:String, sql:String? = rawQuery("SELECT sql FROM sqlite_master WHERE type='table' AND name=?", arrayOf(table)).use { if (it.moveToFirst() && !it.isNull(0)) it.getString(0) else null }): SQLiteTableSummary = SQLiteTableSummary(table, sql, runCatching { countRows(table) }.getOrNull(), columns(table), indexes(table))
    ...
    private suspend fun SQLiteDatabase.countRows(table:String): Long = withTimeout(5_000) { rawQuery("SELECT COUNT(*) FROM ${q.quoteSqliteIdentifier(table)}", null).use { if (it.moveToFirst()) it.getLong(0) else 0L } }
