    ...
    private fun attachGuestView(sessionId: String, host: Activity): GuestViewAttachResult {
        ...
        if (result is GuestViewAttachResult.Success && host.genericPhases.contains("GUEST_ACTIVITY_RESUMED")) {
            ...
            request?.let { req ->
                ...
                state = RuntimeSlotState.ACTIVE_BACKGROUND
                require(ack.sessionChanged == 1 && ack.slotChanged == 1) {
                    "ACTIVE_ACK_REJECTED sessionChanged=${ack.sessionChanged} slotChanged=${ack.slotChanged} attemptId=${req.launchAttemptId}"
                }
                startHeartbeatLoop()
            }
        }
