...
abstract class BaseRuntimeProxyActivity : Activity() {
    ...
    private val heartbeatTick = object : Runnable { override fun run() { sendHeartbeat(if (hasWindowFocus()) "ACTIVE_FOREGROUND" else "ACTIVE_BACKGROUND"); handler.postDelayed(this, 3_000) } }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            recordLaunchHeartbeat(repository, "TOKEN_VALIDATED")
            ...
            recordLaunchHeartbeat(repository, "APK_VERIFIED")
            running = VirtualRuntimeHost(this, repository).start(pkg)
            recordLaunchHeartbeat(repository, "CLASSLOADER_LOADED")
    ...
    private fun recordLaunchHeartbeat(repository: VirtualRepository, phase: String) { runBlocking { repository.db.runtime().heartbeat(sessionId, attemptId, phase, System.currentTimeMillis()) } }
    ...
    override fun onDestroy() { handler.removeCallbacks(heartbeatTick); if (::running.isInitialized) runCatching { running.entry.onDestroy() }; if (isFinishing && !isChangingConfigurations) runCatching { runBlocking { VirtualRepository(applicationContext).db.runtimeSlots().release(slotId.name, System.currentTimeMillis()) } }; super.onDestroy() }
}
